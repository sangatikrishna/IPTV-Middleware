...
const Demo_playlist_saz3 = () => {
  const navigate = useNavigate();
  const theme = useTheme();
  const isNonMediumScreens = useMediaQuery(
    "(min-width: 400px, max-width: 1280px)"
  );
  const [demo_playlist_saz3, setdemo_playlist_saz3] = useState([]);
  const fetchData = () => {
    fetch("http://localhost:5001/api/Demo_playlist_saz3/get")
      .then((res) => res.json())
      .then((res) => {
        console.log(res);
        setdemo_playlist_saz3(res.data);
      });
  };
  ...
  const handleEditAction =        (_id) =>
        ...
        const handleDeleteConfirmation = () => {
          ...
          axios
            .delete(`http://localhost:5001/api/Demo_playlist_saz3/delete/${deleteItemId}`)
            .then((response) => {
