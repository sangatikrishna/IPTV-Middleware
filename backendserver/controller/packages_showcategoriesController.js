...
//GET
const packages_showcategoriesdata =async (req, res) => {
    try {
      const data = await packages_showcategories.find();
  ...
  export default  packages_showcategoriesdata;


//GET by _id
const packages_showcategoriesbyObId=async(req,res)=>
{
try{

  const data=await packages_showcategories.findById(req.params._id)
...
export {packages_showcategoriesbyObId}


//POST API
const addpackages_showcategories=async(req,res)=>
{
...
const {  
  ...
  try{
    ...
    const newData=new packages_showcategories({
        id,
    ...
    await newData.save();
    const videodata = await addpackages_showcategories.find();
    return res.json(await addpackages_showcategories.find());
...
export {addpackages_showcategories}

//DELETE API
const deletepackages_showcategories=async(req,res)=>
{
try{

  const deletedpackages_showcategories=await packages_showcategories.findByIdAndDelete(req.params._id)
  if(!deletedpackages_showcategories) res.status(404).send("No item found")

  // return
  const packages_showcategoriesdata =await packages_showcategories.find()

  res.status(200).json({

    status:"success",
    length:packages_showcategoriesdata.length,
    data:{

        packages_showcategoriesdata}
    });
...
export {deletepackages_showcategories}


const packages_showcategoriesPatch=async(req,res)=>
{
try{
const patchId=req.params._id;

  const updated=await packages_showcategories.findOneAndUpdate({_id:patchId},
   req.body,{new:true})
...
catch(err)
{
  ...
  export{packages_showcategoriesPatch}
