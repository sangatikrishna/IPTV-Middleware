...
import fs from "fs";
import masa_new_app_updates from "../models/masa_new_app_updates.js";

//GET
const masanewappupdatesdata =async (req, res) => {
    try {
      const data = await masa_new_app_updates.find();
  ...
  export default  masanewappupdatesdata;


//GET by _id
const masaNewAppUpdatesbyObId=async(req,res)=>
{
try{

  const data=await masa_new_app_updates.findById(req.params._id)
...
export {masaNewAppUpdatesbyObId}


//POST API
const addMasaNewAppUpdates=async(req,res)=>
{
...
const {  
  ...
  try{
    ...
    const newData=new masa_new_app_updates({
        id,
        version,
        url,
        description,
    ...
    return res.json(await masa_new_app_updates.find());
...
export {addMasaNewAppUpdates}

//DELETE API
const deletemasanewappupdates=async(req,res)=>
{
try{

  const deletedMasaNewAppUpdates=await masa_new_app_updates.findByIdAndDelete(req.params._id)
  if(!deletedMasaNewAppUpdates) res.status(404).send("No item found")
  ...
  res.status(200).json({
    ...
    status:"success",
    length:masanewappupdatesdata.length,
    data:{

        masanewappupdatesdata}
    });
...
export {deletemasanewappupdates}


//PATCH API
const masanewappupdatesPatch = async (req, res) => {
  try {
    const patchId = req.params._id;
 
    const updated = await masa_new_app_updates.findOneAndUpdate(
      { _id: patchId },
      req.body,
      { new: true }
    );
