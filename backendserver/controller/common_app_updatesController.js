...
//GET
const common_app_updatesdata =async (req, res) => {
    try {
      const data = await common_app_updates.find();
  ...
  export default  common_app_updatesdata;


//GET by _id
const common_app_updatesbyObId=async(req,res)=>
{
try{

  const data=await common_app_updates.findById(req.params._id)
...
export {common_app_updatesbyObId}


//POST API
const addcommon_app_updates=async(req,res)=>
{
...
created_date
  ...
  try{
    ...
    const newData=new common_app_updates({
        id,
    ...
    await newData.save();
    const videodata = await addcommon_app_updates.find();
    return res.json(await addcommon_app_updates.find());
...
export {addcommon_app_updates}

//DELETE API
const deletecommon_app_updates=async(req,res)=>
{
try{

  const deletedcommon_app_updates=await common_app_updates.findByIdAndDelete(req.params._id)
  if(!deletedcommon_app_updates) res.status(404).send("No item found")

  // return
  const common_app_updatesdata =await common_app_updates.find()

  res.status(200).json({

    status:"success",
    length:common_app_updatesdata.length,
    data:{

        common_app_updatesdata}
    });
...
export {deletecommon_app_updates}


const common_app_updatesPatch=async(req,res)=>
{
try{
const patchId=req.params._id;

  const updated=await common_app_updates.findOneAndUpdate({_id:patchId},
   req.body,{new:true})
...
catch(err)
{
  ...
  export{common_app_updatesPatch}
