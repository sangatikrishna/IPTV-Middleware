import new_user_product_assignments from "../models/new_user_product_assignments.js";
import dateNow from "date-now";
import date from 'date-and-time';
import fs from "fs";

//GET
const newuserproductassignmentsdata =async (req, res) => {
    try {
      const data = await new_user_product_assignments.find();
  ...
  export default  newuserproductassignmentsdata;


//GET by _id
const newuserproductassignmentsbyObId=async(req,res)=>
{
try{

  const data=await new_user_product_assignments.findById(req.params._id)
...
export {newuserproductassignmentsbyObId}


//POST API
const addnewuserproductassignments=async(req,res)=>
{
...
const {  
  ...
  try{
    ...
    const newData=new new_user_product_assignments({
        id,
    product_id,
    user_id,
    mac_address,
    ...
    modified})
    await newData.save();
    const videodata = await new_user_product_assignments.find();
    return res.json(await new_user_product_assignments.find());
...
export {addnewuserproductassignments}

//DELETE API
const deletenewuserproductassignments=async(req,res)=>
{
try{

  const deletednewuserproductassignments=await new_user_product_assignments.findByIdAndDelete(req.params._id)
  if(!deletednewuserproductassignments) res.status(404).send("No item found")

  // return
  const newuserproductassignmentsdata =await new_user_product_assignments.find()

  res.status(200).json({

    status:"success",
    length:newuserproductassignmentsdata.length,
    data:{

        newuserproductassignmentsdata}
    });
...
export {deletenewuserproductassignments}
const newuserproductassignmentsPatch=async(req,res)=>
...
const patchId=req.params._id;
  const updated=await new_user_product_assignments.findOneAndUpdate({_id:patchId},
...
export{newuserproductassignmentsPatch}
