...
//GET
const packages_audiocategoriesdata =async (req, res) => {
    try {
      const data = await packages_audiocategories.find();
  ...
  export default  packages_audiocategoriesdata;


//GET by _id
const packages_audiocategoriesbyObId=async(req,res)=>
{
try{

  const data=await packages_audiocategories.findById(req.params._id)
...
export {packages_audiocategoriesbyObId}


//POST API
const addpackages_audiocategories=async(req,res)=>
{
...
audio_categories
  ...
  try{
    ...
    const newData=new packages_audiocategories({
        id,
    ...
    await newData.save();
    const videodata = await addpackages_audiocategories.find();
    return res.json(await addpackages_audiocategories.find());
...
export {addpackages_audiocategories}

//DELETE API
const deletepackages_audiocategories=async(req,res)=>
{
try{

  const deletedpackages_audiocategories=await packages_audiocategories.findByIdAndDelete(req.params._id)
  if(!deletedpackages_audiocategories) res.status(404).send("No item found")

  // return
  const packages_audiocategoriesdata =await packages_audiocategories.find()

  res.status(200).json({

    status:"success",
    length:packages_audiocategoriesdata.length,
    data:{

        packages_audiocategoriesdata}
    });
...
export {deletepackages_audiocategories}


const packages_audiocategoriesPatch=async(req,res)=>
{
try{
const patchId=req.params._id;

  const updated=await packages_audiocategories.findOneAndUpdate({_id:patchId},
   req.body,{new:true})
...
catch(err)
{
  ...
  export{packages_audiocategoriesPatch}
