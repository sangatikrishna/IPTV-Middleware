import multi_user_profiles from "../models/multi_user_profiles.js";
import dateNow from "date-now";
import date from 'date-and-time';
import fs from "fs";

//GET
const multiuserprofilesdata =async (req, res) => {
    try {
      const data = await multi_user_profiles.find();
  ...
  export default  multiuserprofilesdata;


//GET by _id
const multiuserprofilesbyObId=async(req,res)=>
{
try{

  const data=await multi_user_profiles.findById(req.params._id)
...
export {multiuserprofilesbyObId}


//POST API
const addmultiuserprofiles=async(req,res)=>
{
...
const {  
  ...
  try{
    ...
    const newData=new multi_user_profiles({
        id,
        user_id,
        address1,
        address2,
        ...
        region_id})
    await newData.save();
    const videodata = await multi_user_profiles.find();
    return res.json(await multi_user_profiles.find());
...
export {addmultiuserprofiles}

//DELETE API
const deletemultiuserprofiles=async(req,res)=>
{
try{

  const deletedmultiuserprofiles=await multi_user_profiles.findByIdAndDelete(req.params._id)
  if(!deletedmultiuserprofiles) res.status(404).send("No item found")

  // return
  const multiuserprofilesdata =await multi_user_profiles.find()

  res.status(200).json({

    status:"success",
    length:multiuserprofilesdata.length,
    data:{

        multiuserprofilesdata}
    });
...
export {deletemultiuserprofiles}

const multiuserprofilesPatch=async(req,res)=>
...
const patchId=req.params._id;
  const updated=await multi_user_profiles.findOneAndUpdate({_id:patchId},
