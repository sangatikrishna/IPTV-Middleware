import new_user_livetv_favorites from "../models/new_user_livetv_favorites.js";
import dateNow from "date-now";
import date from 'date-and-time';
import fs from "fs";

//GET
const newuserlivetvfavoritesdata =async (req, res) => {
    try {
      const data = await new_user_livetv_favorites.find();
  ...
  export default  newuserlivetvfavoritesdata;


//GET by _id
const newuserlivetvfavoritesbyObId=async(req,res)=>
{
try{

  const data=await new_user_livetv_favorites.findById(req.params._id)
...
export {newuserlivetvfavoritesbyObId}


//POST API
const addnewuserlivetvfavorites=async(req,res)=>
{
...
const {  
  ...
  try{
    ...
    const newData=new new_user_livetv_favorites({
        id,
        user_id,
        camera_id,
        created,
        modified})
    await newData.save();
    const videodata = await new_user_livetv_favorites.find();
    return res.json(await new_user_livetv_favorites.find());
...
export {addnewuserlivetvfavorites}

//DELETE API
const deletenewuserlivetvfavorites=async(req,res)=>
{
try{

  const deletednewuserlivetvfavorites=await new_user_livetv_favorites.findByIdAndDelete(req.params._id)
  if(!deletednewuserlivetvfavorites) res.status(404).send("No item found")

  // return
  const newuserlivetvfavoritesdata =await new_user_livetv_favorites.find()

  res.status(200).json({

    status:"success",
    length:newuserlivetvfavoritesdata.length,
    data:{

        newuserlivetvfavoritesdata}
    });
...
export {deletenewuserlivetvfavorites}

const newuserlivetvfavoritesPatch=async(req,res)=>
...
const patchId=req.params._id;
  const updated=await new_user_livetv_favorites.findOneAndUpdate({_id:patchId},
...
export{newuserlivetvfavoritesPatch}
