...
//GET
const packages_modcategoriesdata =async (req, res) => {
    try {
      const data = await packages_modcategories.find();
  ...
  export default  packages_modcategoriesdata;


//GET by _id
const packages_modcategoriesbyObId=async(req,res)=>
{
try{

  const data=await packages_modcategories.findById(req.params._id)
...
export {packages_modcategoriesbyObId}


//POST API
const addpackages_modcategories=async(req,res)=>
{
...
const {  
  ...
  try{
    ...
    const newData=new packages_modcategories({
      id,
    ...
    await newData.save();
    const videodata = await addpackages_modcategories.find();
    return res.json(await addpackages_modcategories.find());
...
export {addpackages_modcategories}

//DELETE API
const deletepackages_modcategories=async(req,res)=>
{
try{

  const deletedpackages_modcategories=await packages_modcategories.findByIdAndDelete(req.params._id)
  if(!deletedpackages_modcategories) res.status(404).send("No item found")

  // return
  const packages_modcategoriesdata =await packages_modcategories.find()

  res.status(200).json({

    status:"success",
    length:packages_modcategoriesdata.length,
    data:{

        packages_modcategoriesdata}
    });
...
export {deletepackages_modcategories}

const packages_modcategoriesPatch=async(req,res)=>
{
try{
const patchId=req.params._id;

  const updated=await packages_modcategories.findOneAndUpdate({_id:patchId},
   req.body,{new:true})
...
catch(err)
{
  ...
  export{packages_modcategoriesPatch}
