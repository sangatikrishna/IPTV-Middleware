...
//GET by _id
const citiesbyObId=async(req,res)=>
{
try{

  const data=await cities.findById(req.params._id)
...
const {  
  ...
  try{
    ...
    await newData.save();
    const videodata = await addcities.find();
    return res.json(await addcities.find());
...
try{
  ...
  const deletedcities=await cities.findByIdAndDelete(req.params._id)
  if(!deletedcities) res.status(404).send("No item found")
...
const citiesPatch=async(req,res)=>
{
try{
const patchId=req.params._id;

  const updated=await cities.findOneAndUpdate({_id:patchId},
   req.body,{new:true})
