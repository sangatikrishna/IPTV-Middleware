...
//GET
const packages_songcategoriesdata =async (req, res) => {
    try {
      const data = await packages_songcategories.find();
  ...
  export default  packages_songcategoriesdata;


//GET by _id
const packages_songcategoriesbyObId=async(req,res)=>
{
try{

  const data=await packages_songcategories.findById(req.params._id)
...
export {packages_songcategoriesbyObId}


//POST API
const addpackages_songcategories=async(req,res)=>
{
...
const {  
  ...
  try{
    ...
    const newData=new packages_songcategories({
        id,
    ...
    await newData.save();
    const videodata = await addpackages_songcategories.find();
    return res.json(await addpackages_songcategories.find());
...
export {addpackages_songcategories}

//DELETE API
const deletepackages_songcategories=async(req,res)=>
{
try{

  const deletedpackages_songcategories=await packages_songcategories.findByIdAndDelete(req.params._id)
  if(!deletedpackages_songcategories) res.status(404).send("No item found")

  // return
  const packages_songcategoriesdata =await packages_songcategories.find()

  res.status(200).json({

    status:"success",
    length:packages_songcategoriesdata.length,
    data:{

        packages_songcategoriesdata}
    });
...
export {deletepackages_songcategories}


const packages_songcategoriesPatch=async(req,res)=>
{
try{
const patchId=req.params._id;

  const updated=await packages_songcategories.findOneAndUpdate({_id:patchId},
   req.body,{new:true})
...
catch(err)
{
  ...
  export{packages_songcategoriesPatch}
