import new_user_profiles from "../models/new_user_profiles.js";
import dateNow from "date-now";
import date from 'date-and-time';
import fs from "fs";

//GET
const newuserprofilesdata =async (req, res) => {
    try {
      const data = await new_user_profiles.find();
  ...
  export default  newuserprofilesdata;


//GET by _id
const newuserprofilesbyObId=async(req,res)=>
{
try{

  const data=await new_user_profiles.findById(req.params._id)
...
export {newuserprofilesbyObId}


//POST API
const addnewuserprofiles=async(req,res)=>
{
...
const {  
  ...
  try{
    ...
    const newData=new new_user_profiles({
        id,
        user_id,
        address1,
        address2,
        ...
        region_id})
    await newData.save();
    const videodata = await new_user_profiles.find();
    return res.json(await new_user_profiles.find());
...
export {addnewuserprofiles}

//DELETE API
const deletenewuserprofiles=async(req,res)=>
{
try{

  const deletednewuserprofiles=await new_user_profiles.findByIdAndDelete(req.params._id)
  if(!deletednewuserprofiles) res.status(404).send("No item found")

  // return
  const newuserprofilesdata =await new_user_profiles.find()

  res.status(200).json({

    status:"success",
    length:newuserprofilesdata.length,
    data:{
...
export {deletenewuserprofiles}
const newuserprofilesPatch=async(req,res)=>
...
const patchId=req.params._id;
  const updated=await new_user_profiles.findOneAndUpdate({_id:patchId},
