...
import fs from "fs";
import masa_offer_messages from "../models/masa_offer_messages.js";

//GET
const masaoffermessagesdata =async (req, res) => {
    try {
      const data = await masa_offer_messages.find();
  ...
  export default  masaoffermessagesdata;


//GET by _id
const masaOfferMessagesbyObId=async(req,res)=>
{
try{

  const data=await masa_offer_messages.findById(req.params._id)
...
export {masaOfferMessagesbyObId}


//POST API
const addMasaOfferMessages=async(req,res)=>
{
...
const {  
  ...
  try{
    ...
    const newData=new masa_offer_messages({
        id,
  title,
  description,
  status,
  ...
  updated,})
    ...
    return res.json(await masa_offer_messages.find());
...
export {addMasaOfferMessages}

//DELETE API
const deletemasaoffermessages=async(req,res)=>
{
try{

  const deletedMasaOfferMessages=await masa_offer_messages.findByIdAndDelete(req.params._id)
  if(!deletedMasaOfferMessages) res.status(404).send("No item found")
  ...
  res.status(200).json({
    ...
    status:"success",
    length:masaoffermessagesdata.length,
    data:{

        masaoffermessagesdata}
    });
...
export {deletemasaoffermessages}


//PATCH API
const masaoffermessagesPatch = async (req, res) => {
  try {
    const patchId = req.params._id;
 
    const updated = await masa_offer_messages.findOneAndUpdate(
      { _id: patchId },
      req.body,
      { new: true }
    );
