import multi_ver_app_updates from "../models/multi_ver_app_updates.js";
import dateNow from "date-now";
import date from 'date-and-time';
import fs from "fs";

//GET
const multiverappupdatesdata =async (req, res) => {
    try {
      const data = await multi_ver_app_updates.find();
  ...
  export default  multiverappupdatesdata;


//GET by _id
const multiverappupdatesbyObId=async(req,res)=>
{
try{

  const data=await multi_ver_app_updates.findById(req.params._id)
...
export {multiverappupdatesbyObId}


//POST API
const addmultiverappupdates=async(req,res)=>
{
...
const {  
  ...
  try{
    ...
    const newData=new multi_ver_app_updates({
    id,
    version,
    url,
    description,
    ...
    created_date})
    await newData.save();
    const videodata = await multi_ver_app_updates.find();
    return res.json(await multi_ver_app_updates.find());
...
export {addmultiverappupdates}

//DELETE API
const deletemultiverappupdates=async(req,res)=>
{
try{

  const deletedmultiverappupdates=await multi_ver_app_updates.findByIdAndDelete(req.params._id)
  if(!deletedmultiverappupdates) res.status(404).send("No item found")

  // return
  const multiverappupdatesdata =await multi_ver_app_updates.find()

  res.status(200).json({

    status:"success",
    length:multiverappupdatesdata.length,
    data:{

        multiverappupdatesdata}
    });
...
export {deletemultiverappupdates}

const multiverappupdatesPatch=async(req,res)=>
...
const patchId=req.params._id;
  const updated=await multi_ver_app_updates.findOneAndUpdate({_id:patchId},
...
export{multiverappupdatesPatch}
