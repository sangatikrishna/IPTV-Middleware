...
//GET
const payment_settingsdata =async (req, res) => {
    try {
      const data = await payment_settings.find();
  ...
  export default  payment_settingsdata;


//GET by _id
const payment_settingsbyObId=async(req,res)=>
{
try{

  const data=await payment_settings.findById(req.params._id)
...
export {payment_settingsbyObId}


//POST API
const addpayment_settings=async(req,res)=>
{
...
const {  
 ...
 modified
  ...
  try{
    ...
    const newData=new payment_settings({
        id,
        key,
        value,
        type,
    ...
    await newData.save();
    const videodata = await addpayment_settings.find();
    return res.json(await addpayment_settings.find());
...
export {addpayment_settings}

//DELETE API
const deletepayment_settings=async(req,res)=>
{
try{

  const deletedpayment_settings=await payment_settings.findByIdAndDelete(req.params._id)
  if(!deletedpayment_settings) res.status(404).send("No item found")

  // return
  const payment_settingsdata =await payment_settings.find()

  res.status(200).json({

    status:"success",
    length:payment_settingsdata.length,
    data:{

        payment_settingsdata}
    });
...
export {deletepayment_settings}

// patch

const payment_settingsPatch=async(req,res)=>
{
try{
const patchId=req.params._id;

  const updated=await payment_settings.findOneAndUpdate({_id:patchId},
   req.body,{new:true})
