...
//GET
const packagesdata =async (req, res) => {
    try {
...
//GET by _id
const packagesbyObId=async(req,res)=>
{
try{

  const data=await packages.findById(req.params._id)
...
status
  ...
  try{
    ...
    const newData=new packages({
      ...
      status})
    await newData.save();
    const videodata = await packages.find();
    return res.json(await packages.find());
...
try{
  ...
  const deletedpackages=await packages.findByIdAndDelete(req.params._id)
  if(!deletedpackages) res.status(404).send("No item found")
...
export {deletepackages}

// // patch

const  packagesPatch=async(req,res)=>
...
const patchId=req.params._id;
  ...
  const updated=await packages.findOneAndUpdate({_id:patchId},
