...
import fs from "fs";
import adons_categories from "../models/adons_categories.js";

//GET
const adonscategoriesdata =async (req, res) => {
    try {
      const data = await adons_categories.find();
  ...
  export default  adonscategoriesdata;


//GET by _id
const adonsCategoriesbyObId=async(req,res)=>
{
try{

  const data=await adons_categories.findById(req.params._id)
...
export {adonsCategoriesbyObId}


//POST API
const addAdonsCategories=async(req,res)=>
{
...
const {  
  ...
  try{
    ...
    const newData=new adons_categories({
        id,
        title,
        image,
        created,
    ...
    return res.json(await adons_categories.find());
...
export {addAdonsCategories}

//DELETE API
const deleteadonscategories=async(req,res)=>
{
try{

  const deletedAdonsCategories=await adons_categories.findByIdAndDelete(req.params._id)
  if(!deletedAdonsCategories) res.status(404).send("No item found")
  ...
  res.status(200).json({
    ...
    status:"success",
    length:adonscategoriesdata.length,
    data:{
...
export {deleteadonscategories}


//PATCH API
const adonscategoriesPatch = async (req, res) => {
  try {
    const patchId = req.params._id;
 
    const updated = await adons_categories.findOneAndUpdate(
      { _id: patchId },
      req.body,
      { new: true }
    );
