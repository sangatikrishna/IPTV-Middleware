...
//GET
const packages_videocategoriesdata =async (req, res) => {
    try {
      const data = await packages_videocategories.find();
  ...
  export default  packages_videocategoriesdata;


//GET by _id
const packages_videocategoriesbyObId=async(req,res)=>
{
try{

  const data=await packages_videocategories.findById(req.params._id)
...
export {packages_videocategoriesbyObId}


//POST API
const addpackages_videocategories=async(req,res)=>
{
...
const {  
  ...
  try{
    ...
    const newData=new packages_videocategories({
        id,
    ...
    await newData.save();
    const videodata = await addpackages_videocategories.find();
    return res.json(await addpackages_videocategories.find());
...
export {addpackages_videocategories}

//DELETE API
const deletepackages_videocategories=async(req,res)=>
{
try{

  const deletedpackages_videocategories=await packages_videocategories.findByIdAndDelete(req.params._id)
  if(!deletedpackages_videocategories) res.status(404).send("No item found")

  // return
  const packages_videocategoriesdata =await packages_videocategories.find()

  res.status(200).json({

    status:"success",
    length:packages_videocategoriesdata.length,
    data:{

        packages_videocategoriesdata}
    });
...
export {deletepackages_videocategories}


const packages_videocategoriesPatch=async(req,res)=>
{
try{
const patchId=req.params._id;

  const updated=await packages_videocategories.findOneAndUpdate({_id:patchId},
   req.body,{new:true})
...
catch(err)
{
  ...
  export{packages_videocategoriesPatch}
