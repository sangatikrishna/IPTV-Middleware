...
//GET
const catchupprogramsdata =async (req, res) => {
    try {
      const data = await catchup_programs.find();
  ...
  export default  catchupprogramsdata;


//GET by _id
const catchupprogramsbyObId=async(req,res)=>
{
try{

  const data=await catchup_programs.findById(req.params._id)
...
export {catchupprogramsbyObId}


//POST API
const addcatchupprograms=async(req,res)=>
{
...
const {  
  ...
  try{
    ...
    const newData=new catchup_programs({
        id,
        title,
        description,
        program_url,
        ...
        camera_id})
    await newData.save();
    const videodata = await catchup_programs.find();
    return res.json(await catchup_programs.find());
...
export {addcatchupprograms}

//DELETE API
const deletecatchupprograms=async(req,res)=>
{
try{

  const deletedcatchupprograms=await catchup_programs.findByIdAndDelete(req.params._id)
  if(!deletedcatchupprograms) res.status(404).send("No item found")
  ...
  res.status(200).json({
    ...
    status:"success",
    length:catchupprogramsdata.length,
    data:{
...
export {deletecatchupprograms}
const catchupprogramsPatch=async(req,res)=>
...
const patchId=req.params._id;
  const updated=await catchup_programs.findOneAndUpdate({_id:patchId},
