...
//GET
const packages_vodcategoriesdata =async (req, res) => {
    try {
      const data = await packages_vodcategories.find();
  ...
  export default  packages_vodcategoriesdata;


//GET by _id
const packages_vodcategoriesbyObId=async(req,res)=>
{
try{

  const data=await packages_vodcategories.findById(req.params._id)
...
export {packages_vodcategoriesbyObId}


//POST API
const addpackages_vodcategories=async(req,res)=>
{
...
const {  
  ...
  try{
    ...
    const newData=new packages_vodcategories({
        id,
        package_id,
        categories_id
    ...
    await newData.save();
    const videodata = await addpackages_vodcategories.find();
    return res.json(await addpackages_vodcategories.find());
...
export {addpackages_vodcategories}

//DELETE API
const deletepackages_vodcategories=async(req,res)=>
{
try{

  const deletedpackages_vodcategories=await packages_vodcategories.findByIdAndDelete(req.params._id)
  if(!deletedpackages_vodcategories) res.status(404).send("No item found")

  // return
  const packages_vodcategoriesdata =await packages_vodcategories.find()

  res.status(200).json({

    status:"success",
    length:packages_vodcategoriesdata.length,
    data:{

        packages_vodcategoriesdata}
    });
...
export {deletepackages_vodcategories}


const packages_vodcategoriesPatch=async(req,res)=>
{
try{
const patchId=req.params._id;

  const updated=await packages_vodcategories.findOneAndUpdate({_id:patchId},
   req.body,{new:true})
...
catch(err)
{
  ...
  export{packages_vodcategoriesPatch}
