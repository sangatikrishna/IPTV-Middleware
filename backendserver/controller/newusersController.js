import new_users from "../models/new_users.js";
import dateNow from "date-now";
import date from 'date-and-time';
import fs from "fs";

//GET
const newusersdata =async (req, res) => {
    try {
      const data = await new_users.find();
...
//GET by _id
const newusersbyObId=async(req,res)=>
{
try{

  const data=await new_users.findById(req.params._id)
...
const {  
  ...
  try{
    ...
    const newData=new new_users({
        ...
        reseller_id})
    await newData.save();
    const videodata = await new_users.find();
    return res.json(await new_users.find());
...
//DELETE API
const deletenewusers=async(req,res)=>
{
try{

  const deletednewusers=await new_users.findByIdAndDelete(req.params._id)
  if(!deletednewusers) res.status(404).send("No item found")

  // return
  const newusersdata =await new_users.find()

  res.status(200).json({

    status:"success",
    length:newusersdata.length,
    data:{
...
export {deletenewusers}
const newusersPatch=async(req,res)=>
...
const patchId=req.params._id;
  const updated=await new_users.findOneAndUpdate({_id:patchId},
