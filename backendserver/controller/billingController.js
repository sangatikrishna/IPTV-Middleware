...
//GET by _id
const billingbyObId=async(req,res)=>
{
try{

  const data=await billing.findById(req.params._id)
...
const {  
  ...
  try{
    ...
    const newData=new billing({
      id,
  billingType_id,
  price,
  package_id})
    await newData.save();
    const videodata = await billing.find();
    return res.json(await billing.find());
...
try{
  ...
  const deletedbilling=await billing.findByIdAndDelete(req.params._id)
  if(!deletedbilling) res.status(404).send("No item found")
...
const billingPatch=async(req,res)=>
...
const patchId=req.params._id;
  const updated=await billing.findOneAndUpdate({_id:patchId},
